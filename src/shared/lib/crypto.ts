export const encryptKey = async (token: string) => {
    ...
    const key = await crypto.subtle.importKey("raw", keyData, "AES-GCM", true, ["encrypt", "decrypt"]);
...
export const decryptKey = async (token?: string) => {
    try {
        if (!token) {
            return;
        }
